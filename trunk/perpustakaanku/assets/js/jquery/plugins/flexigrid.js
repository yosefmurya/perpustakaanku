...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 useRo: false, //refresh option
			 useOutSideFunction: false,
			 outSideFName : false, //update handle
			 refreshOptions: [10,20,30,40,50,60],			 
			 showFooterMenu: true,
			 my_name : ''
		  }, p);
...
/*			
			...
			{
			var n = 
			$('thead tr:first th:visible',g.hDiv).index(this);
...
*/
			$('thead tr:first th:visible',this.hDiv).each
				(
			 	function ()
					...
					if($.browser.msie()==false && $.browser.firefox()==false){
						cdpos = cdpos + cdleft + cdpad;
					}
		...
		//error in ie don't know why ? 11/3/2010 - satiri
		...
		*/
		...
		{
			this_name = p.my_name;
			test_timer = p.refreshRate;
			this_rate = p.refreshRate;
		
			$('.pReload',g.pDiv).everyTime((p.refreshRate*1000), function()
			{
				if(p.useOutSideFunction==true)
				{
					if(p.outSideFName!=false){	
						eval(p.outSideFName);
					}
				}else{
			...
			$('.pReload',g.pDiv).everyTime(1000, function()
			{
				if(this_name!=null){
					if(test_timer <= 0){
						test_timer = this_rate;
					}else{					
						//not shown due to inconcistency
						//$("#refreshText_" + this_name).html("Will Refresh in : " + test_timer.toString() + "s " + this_name);
						test_timer--;
					}
				}
			});//timer autorefresh
					 	...
					 	function ()
							{
								...
								// fixing ie 6 bug
								if ( $('.vsble').height() == 16 ) 
									$('.flexigrid').css('min-height', 16);
								else 
									$('.flexigrid').css('min-height', $('.flexigrid').height());
								// end fixing ie 6 bug
							}
					);
				}
...
//countdown timer
			...
			{
				var opt = "";
				for (var nx=0;nx<p.refreshOptions.length;nx++)
				{
					if (p.rp == p.refreshOptions[nx]) sel = 'selected="selected"'; else sel = '';
					 opt += "<option value='" + p.refreshOptions[nx] + "' " + sel + " >" + p.refreshOptions[nx] + "&nbsp;&nbsp;</option>";
				};
				$('.pDiv2',g.pDiv).append("<div class='btnseparator'></div><div class='pGroup'>Refresh Rate : <select name='refreshOption'>"+opt+"</select></div><div id='refreshText_"+p.my_name+"' style='top:0px'></div>");
				$('select',g.pDiv).change(
						function ()
						{
							if (p.onRefreshChange) 
								p.onRefreshChange(+this.value);
							else
								{
								p.newp = 1;
								p.refreshRate = +this.value;
								g.populate();
								}
						}
					);
